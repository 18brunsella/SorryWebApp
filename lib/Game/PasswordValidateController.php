...
class PasswordValidateController extends Controller {
    ...
    public function __construct(Site $site, array $post) {
        ...
        if(isset($post['ok'])) {
            ...
            if ($userid === null) {
                $this->redirect = "$root/password-validate.php?v=$validator&e=".PasswordValidateView::INVALID;
                return;
            }
            //
            ...
            if ($editUser === null) {
                // User does not exist!
                $this->redirect = "$root/password-validate.php?v=$validator&e=".PasswordValidateView::NO_USER_EXISTS;
                return;
            }
            $email = trim(strip_tags($post['email']));
            if ($email !== $editUser->getEmail()) {
                // Email entered is invalid
                $this->redirect = "$root/password-validate.php?v=$validator&e=".PasswordValidateView::NO_VALID_MATCH;
                return;
            }
            ...
            if ($password1 !== $password2) {
                ...
                $this->redirect = "$root/password-validate.php?v=$validator&e=".PasswordValidateView::NO_PASSWORD_MATCH;
                return;
            }
